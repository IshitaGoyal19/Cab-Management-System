...
class customer
{
    ...
    public: customer()
            ...
            void getride(int **a, downlinks *l, int n, char loc[])
            {
                trides++;
                strcpy(r->sloc, loc);
                r->cabb=check(r->sloc);
                ...
                tm *ltm = localtime(&now);
				 ...
				 r->id=retcustid();
				 //customer c;
                 fstream f("rides.dat",ios::binary|ios::out|ios::app);
                 if(!f)
                    {
                        cout<<"\t CAN'T PROCEED\t";
                        getch();
...
void aridedisplay()
{
 ...
 cout<<"\n  \n\n";
cout<<"      ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ CAB'S RIDES ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^\n\n";
 cout<<setw(15)<<"ID"<<setw(20)<<"START POINT"<<setw(20)<<"DESTINATION"<<setw(17)<<"DISTANCE"<<setw(17)<<"FARE"<<setw(18)<<"RIDE DATE"<<setw(18)<<"CUSTOMER ID\n\n";
 cout<<"      -------------------------------------------------------------------------------------------------------------\n\n";
  if(!f)
{
       cout<<"\t no record";
...
cin>>id;
  while(f)
 {
  ...
  if(id==r.cabb->retcid())
  {
      cout<<setw(10)<<r.cabb->retcid()<<setw(25)<<r.sloc<<setw(25)<<r.floc<<setw(18)<<r.distance<<setw(16)<<r.fare<<setw(18)<<r.dd<<"-"<<r.mm<<"-"<<r.yyyy<<setw(15)<<id;
  }
 }
 getch();
 return;
...
cout<<"      ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ CUSTOMER'S RIDES ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^\n\n";
 cout<<setw(15)<<"ID"<<setw(20)<<"START POINT"<<setw(20)<<"DESTINATION"<<setw(17)<<"DISTANCE"<<setw(17)<<"FARE"<<setw(18)<<"RIDE DATE"<<setw(18)<<"CAB ID\n\n";
 cout<<"      -------------------------------------------------------------------------------------------------------------\n\n";
  if(!f)
{
       cout<<"\t no record";
...
cin>>id;
  while(f)
 {
  ...
  if(id==r.id)
  {
      cout<<setw(10)<<r.id<<setw(25)<<r.sloc<<setw(25)<<r.floc<<setw(18)<<r.distance<<setw(16)<<r.fare<<setw(18)<<r.dd<<"-"<<r.mm<<"-"<<r.yyyy<<setw(15)<<r.cabb->retcid();
  }
 }
 getch();
 return;
